...
def downsample_data(X_titles, X_contexts, y):
    # Combine titles and contexts into a single structure (list of tuples)
    combined_data = list(zip(X_titles, X_contexts, y))

    # Initialize RandomUnderSampler
    rus = RandomUnderSampler(sampling_strategy='auto', random_state=42)

    # Apply undersampling on the combined data (titles + contexts + labels)
    X_combined, y_resampled = rus.fit_resample(combined_data, y)

    # Separate the undersampled titles, contexts, and labels
    X_titles_resampled = [item[0] for item in X_combined]
    X_contexts_resampled = [item[1] for item in X_combined]

    # Print the number of records after downsampling
    print(f"Number of records after downsampling: {len(X_titles_resampled)}")

    return X_titles_resampled, X_contexts_resampled, y_resampled
...
# Apply downsampling for titles and contexts together
print("Perform Downsampling......")
titles_downsampled, contexts_downsampled, labels_downsampled = downsample_data(title_texts, context_texts, labels)

# Initialize and fit tokenizer
tokenizer = Tokenizer()
tokenizer.fit_on_texts(titles_downsampled+contexts_downsampled)
...
X_title_train, X_title_val, X_context_train, X_context_val, y_train, y_val = train_test_split(
    padded_titles, padded_contexts, labels_downsampled,
    test_size=0.2,
    random_state=42,
    stratify=labels_downsampled
)
