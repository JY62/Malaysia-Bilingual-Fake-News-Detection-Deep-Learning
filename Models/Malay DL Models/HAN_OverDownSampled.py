...
print(f"Real: {sum(labels == 1)}")
print(f"Fake: {sum(labels == 0)}")

# Apply SMOTE for oversampling the minority class
smote = SMOTE(random_state=42)
title_embeddings_resampled, labels_resampled = smote.fit_resample(title_embeddings, labels)
context_embeddings_resampled, _ = smote.fit_resample(context_embeddings, labels)

# Display counts after SMOTE oversampling
print("Counts after SMOTE oversampling:")
print(f"Real: {sum(labels_resampled == 1)}")
print(f"Fake: {sum(labels_resampled == 0)}")

# Split data
X_title_train, X_title_val, X_context_train, X_context_val, y_train, y_val = train_test_split(
    title_embeddings_resampled, context_embeddings_resampled, labels_resampled, test_size=0.2, random_state=42,
